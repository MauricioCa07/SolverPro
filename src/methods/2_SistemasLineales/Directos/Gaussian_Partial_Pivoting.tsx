...
function GaussianPartialPivoting({
  matrixA,
  vectorB,
  n,
}: {
  ...
  useEffect(() => {
    try {
      let A = math.matrix(
        matrixA.split(";").map((row) => row.split(",").map(Number))
      );
      ...
      if (Math.abs(math.det(A)) < 1e-10) {
        throw new Error(
          "Matrix is singular or nearly singular. Cannot proceed."
        );
      }
      ...
      for (let k = 0; k < nVar - 1; k++) {
        ...
        for (let i = k + 1; i < nVar; i++) {
          if (
            Math.abs(augmentedMatrix.get([i, k])) >
            Math.abs(augmentedMatrix.get([maxIndex, k]))
          ) {
            maxIndex = i;
          }
        }
        ...
        for (let i = k + 1; i < nVar; i++) {
          let factor =
            augmentedMatrix.get([i, k]) / augmentedMatrix.get([k, k]);
          for (let j = k; j <= nVar; j++) {
            let value =
              augmentedMatrix.get([i, j]) -
              factor * augmentedMatrix.get([k, j]);
      ...
      for (let i = nVar - 1; i >= 0; i--) {
        ...
        x[i] =
          (augmentedMatrix.get([i, nVar]) - sum) / augmentedMatrix.get([i, i]);
  ...
  const plotData = solution.map((value, index) => ({
    x: `x${index + 1}`,
    value,
  }));
